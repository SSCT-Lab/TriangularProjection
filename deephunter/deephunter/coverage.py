...
class Coverage():
    ...
    def pt_update_coverage_batch(self, outputs, ptr, n=10, deep_num=4):
        '''
                ...
                '''
        # ptr = np.zeros((self.total_size,)).astype("uint8")
        for idx, layer_name in enumerate(self.layer_to_compute):
            if idx != len(self.layer_to_compute) - 1:
                continue
            else:
                layer_outputs = outputs[idx]
                if self.matrix is None:
                    coverage = pt.TriProCover().cal_triangle_cov(layer_outputs, np.argmax(layer_outputs, axis=-1), n,
                                                                 deep_num=deep_num,
                                                                 by_deep_num=False)
                else:
                    temp_matrix = np.concatenate([layer_outputs, self.matrix])
                    coverage = pt.TriProCover().cal_triangle_cov(temp_matrix, np.argmax(temp_matrix, axis=-1), n,
                                                                 deep_num=deep_num,
                                                                 by_deep_num=False)
                block = int(self.total_size * coverage[0])
                if block > self.cur_block:
                    self.cur_block = block
                    if self.matrix is None:
                        self.matrix = layer_outputs
                    else:
                ...
                for j in range(len(layer_outputs)):
                    for i in range(block):
                        if i < len(ptr[0]):
                            # print(j,i)
                            ptr[j][i] = 1
                # print("ptr",block, np.sum(ptr[0]))

    def pt_update_coverage_increase(self, outputs, ptr, n=10, deep_num=4):
        self.update_block(n, deep_num)
        '''
                Given the input, update the neuron covered in the model by this input.
                    This includes mark the neurons covered by this input as "covered"
                :param input_data: the input image
        ...
        for idx, layer_name in enumerate(self.layer_to_compute):
            if idx != len(self.layer_to_compute) - 1:
                ...
                block_arr = [self.cur_block]
                layer_outputs = outputs[idx]
                flag_arr = np.zeros((len(layer_outputs),))
                flag2_map = {}
                for ix, layer_output in enumerate(layer_outputs):
                    if self.matrix is None:
                        temp_matrix = np.array([layer_output])
                    else:
                        temp_matrix = np.concatenate([[layer_output], self.matrix])
                    coverage = pt.TriProCover().cal_triangle_cov(temp_matrix, np.argmax(temp_matrix, axis=-1), n,
                                                                 deep_num=deep_num,
                                                                 by_deep_num=False)
                    block = int(self.total_size * coverage[0])
                    ...
                    if block > self.cur_block:
                        flag_arr[ix] = 1
                        if self.matrix is None:
                            self.matrix = np.array([layer_output])
                        else:
                            self.matrix = temp_matrix
                        flag2_map[ix] = len(self.matrix) - 1
                self.cur_block = np.max(block_arr)
                self.flag2_map = flag2_map
                # print(self.matrix.shape, self.cur_block)
                # print(self.flag2_map)
                for j in range(len(layer_outputs)):
                    if flag_arr[j] == 1:
                        for i in range(self.cur_block):
                            if i < len(ptr[0]):
                                # print(j,i)
                                ptr[j][i] = 1
                # print("ptr",block, np.sum(ptr[0]))

    def update_block(self, n, deep_num):
        if self.matrix is not None:
            coverage = pt.TriProCover().cal_triangle_cov(self.matrix, np.argmax(self.matrix, axis=-1), n,
                                                         deep_num=deep_num,
                                                         by_deep_num=False)
            block = int(self.total_size * coverage[0])
        else:
    ...
    def update_matrix(self, index):
        ix_arr = []
        for ix in index:
            if ix in self.flag2_map.keys():
                ix_arr.append(self.flag2_map[ix])
        self.matrix = np.delete(self.matrix, ix_arr, axis=0).copy()
    ...
    def update_coverage(self, outputs):
        ...
        if len(outputs) > 0 and len(outputs[0]) > 0:
            ...
            elif self.criteria == 'pt':
                # fuzz procession higher coverage
                self.pt_update_coverage_increase(outputs, ptr)
            elif self.criteria == 'pt_bh':
                # fuzz procession lower resources
                self.pt_update_coverage_batch(outputs, ptr)
