...
class ImageInputCorpus(FuzzQueue):
    ...
    def __init__(self, outdir, israndom, sample_function, cov_num, criteria, coverage_handler=None):
        """Init the class.
        ...
        Returns:
          Initialized object.
        """
        FuzzQueue.__init__(self, outdir, israndom, sample_function, cov_num, criteria, coverage_handler)

    def save_if_interesting(self, seed, data, crash, dry_run=False, suffix=None, batch_inc=False, is_empty=False):
        ...
        if crash:
            ...
            if self.has_new_bits(seed) or batch_inc or dry_run:
                self.last_reg_time = current_time
                seed.queue_time = current_time
                seed.id = self.total_queue
                # the seed path
