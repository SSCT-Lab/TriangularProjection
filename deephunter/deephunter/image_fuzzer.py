...
model_weight_path = {
    'vgg16': "./profile/cifar10/models/vgg16.hdf5",
    'resnet20': "./profile/cifar10/models/resnet.hdf5",
    'lenet1': "./profile/mnist/models/lenet1.hdf5",
    'lenet4': "./profile/mnist/models/lenet4.hdf5",
    'lenet5': "./profile/mnist/models/lenet5.hdf5"
}
    ...
    def func(queue, root_seed, parent, mutated_coverage_list, mutated_data_batches, mutated_metadata_list,
        ...
        for idx in range(len(mutated_coverage_list)):
            ...
            if len(results) > 0:
                ...
                is_empty = np.sum(input.coverage) == 0
                if args.criteria == "pt":
                    batch_inc = True
                result = queue.save_if_interesting(input, new_img, False, batch_inc=batch_inc, is_empty=is_empty)
                if args.criteria == "pt_bh":
                    if result:
                        batch_inc = True
                successed = successed or result
        if args.criteria == "pt":
            queue.coverage_handler.update_matrix(del_idx)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-criteria', help="set the criteria to guide the fuzzing",
                        choices=['nc', 'kmnc', 'nbc', 'snac', 'bknc', 'tknc', 'fann', 'pt','pt_bh'], default='kmnc')
    parser.add_argument('-batch_num', help="the number of mutants generated for each seed", type=int, default=20)
    parser.add_argument('-max_iteration', help="maximum number of fuzz iterations", type=int, default=1000000)
    ...
    fuzzer = Fuzzer(queue, coverage_function, metadata_function, objective_function, mutation_function, fetch_function,
                    image_iterate_function, args.select, coverage_handler=coverage_handler)
